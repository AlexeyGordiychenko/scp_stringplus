...
char *scp_strtok(char *str, const char *delim) {
  ...
  static char *saveptr =
      SCP_NULL;  // сохраняет состояние между вызовами функции

  if (str != SCP_NULL) {  // если указатель на строку не равен NULL
    saveptr = str;  // обновляем указатель на начало строки
  } else {
    if (saveptr ==
        SCP_NULL) {  // если строка закончилась и нет сохраненного указателя
      return SCP_NULL;  // возвращаем NULL
    }
    str = saveptr;  // начинаем обработку той же строки, с которой закончили
  }

  str += scp_strspn(str, delim);  // пропускаем символы-разделители

  if (*str == '\0') {  // если строка закончилась
    saveptr = SCP_NULL;  // обнуляем указатель на следующий токен
    return SCP_NULL;  // возвращаем NULL
  }

  char *end = str + scp_strcspn(str, delim);  // находим конец текущего токена
