...
bool scp_isdigit(int a) { return (a >= '0' && a <= '9'); }

bool scp_isspace(int c) {
  return (c == ' ' || c == '\t' || c == '\n' || c == '\r' || c == '\f' ||
          c == '\v');
}
...
bool str_to_int(const char **str, long long *value, scp_size_t width,
                int base) {
  int sign = 1;
  long long result = 0;
  bool overflow = false;
...
bool str_to_uint(const char **str, unsigned long long *value, scp_size_t width,
                 int base) {
  int sign = 1;
...
bool str_to_float(const char **str, long double *value, scp_size_t width) {
  int sign = 1, base = 10;
  scp_size_t count = 0, decimal_places = 0;
...
bool parse_pre_number(scp_size_t *count, int *sign, scp_size_t width, int *base,
                      const char **str) {
  bool res = false;

  // skip whitespace
  while (scp_isspace(**str)) {
    (*str)++;
  }
...
bool get_nan_inf(const char **str, scp_size_t width, scp_size_t count, int sign,
                 long double *value) {
  bool res = false;
  char tmp[9] = {'\0'};
  char *tmp_lower = scp_to_lower(scp_strncpy(tmp, *str, 8));
  if ((count + 8 <= width || width == 0) &&
      scp_strncmp(tmp_lower, "infinity", 8) == 0) {
    (*str) += 8;
    *value = sign * INFINITY;
    res = true;
  } else if (count + 3 <= width || width == 0) {
    if (scp_strncmp(tmp_lower, "nan", 3) == 0) {
      (*str) += 3;
      *value = sign * NAN;
      res = true;
    } else if (scp_strncmp(tmp_lower, "inf", 3) == 0) {
      (*str) += 3;
      *value = sign * INFINITY;
      res = true;
    }
...
bool str_to_float_overflow(const char **str, long double *value,
                           scp_size_t width, scp_size_t *count, int *sign,
                           int base) {
