...
void execute_x(char **p, va_list *args, Flag flags) {
  ...
  int hex_len = (int)scp_strlen(hex);
  scp_strncpy(*p, hex, hex_len);
  (*p) += hex_len;
}
...
void process_d_spec(Flag flags, va_list *args, char **p) {
  ...
  if (flags.width > len && !flags.minus) {
    scp_memset(*p, (flags.zero) ? '0' : ' ', flags.width - len);
    (*p) += flags.width - len;
  }
  ...
  scp_strncpy(*p, temp, temp_size);
  (*p) += len - ((sign == '\0') ? 0 : 1);
  if (flags.width > len && flags.minus) {
    scp_memset(*p, ' ', flags.width - len);
    (*p) += flags.width - len;
  }
}
...
void apply_flags(char *str, Flag flags) {
  ...
  if (flags.precision != -1 && flags.spec != 'f' && flags.spec != 'g' &&
    ...
    if (flags.precision == 0 && !scp_strncmp(str, "0", 1)) {
      str[0] = '\0';
    } else {
      int for_prefix = 0;
      ...
      while ((int)scp_strlen(str) < flags.precision - for_prefix) {
  ...
  // обработка #
  if ((flags.prefix && scp_strncmp(str, "0", 2) && str[0] != '\0') ||
      flags.spec == 'p') {  // флаг #
    ...
    if ((flags.spec == 'f') && !scp_strchr(str, '.')) {
      int len = scp_strlen(str);
      str[len] = '.';
      str[len + 1] = '\0';
    }
    ...
    if ((flags.spec == 'E' || flags.spec == 'e' || flags.spec == 'G' ||
      ...
      if (flags.spec == 'e' || flags.spec == 'g')
        position = scp_strchr(str, 'e');
      if (flags.spec == 'E' || flags.spec == 'G')
        position = scp_strchr(str, 'E');

      if (position != SCP_NULL) {
        int index = position - str;  // Вычисляем индекс первого вхождения
        int len = (int)scp_strlen(str);
  ...
  if (flags.width != 0 && !flags.minus) {
    char ch = ' ';
    if (flags.zero) ch = '0';
    while ((int)scp_strlen(str) < flags.width) {
...
void execute_u(char **p, va_list *args, Flag flags) {
  ...
  int udigit_len = (int)scp_strlen(udigit);
  scp_strncpy(*p, udigit, udigit_len);
  (*p) += udigit_len;
}
...
int process_s_spec(Flag flags, va_list *args, char **p) {
  scp_size_t len = 0;
  scp_size_t width = (scp_size_t)flags.width;
  scp_size_t precision =
      (flags.precision < 0) ? 0 : (scp_size_t)flags.precision;
  bool wchar = (flags.length == 'l') ? true : false;
  bool null_line = false;
  int res = 0;

  wchar_t *wline = SCP_NULL;
  const char *line = SCP_NULL;
  ...
  if (width > len && !flags.minus) {
    scp_memset(*p, ' ', width - len);
    (*p) += width - len;
  }

  if (wchar) {
    for (scp_size_t i = 0; i < len; i++) {
      res += put_wchar(p, wline[i]);
    }
  } else {
    scp_strncpy(*p, line, len);
    (*p) += len;
  }
...
void execute_p(char **p, va_list *args, Flag flags) {
...
#ifdef __linux__
  if (ptr == 0) {
    scp_strncpy(buffer, "(nil)", 5);
    buffer[5] = '\0';
    if (flags.width != 0 && !flags.minus) {
      char ch = ' ';
      if (flags.zero) ch = '0';
      while ((int)scp_strlen(buffer) < flags.width) {
        input_char_left(buffer, ch);
      }

    } else if (flags.width != 0 && flags.minus) {
      while ((int)scp_strlen(buffer) < flags.width) {
        int len = (int)scp_strlen(buffer);
        buffer[len] = ' ';
        buffer[len + 1] = '\0';
      }
    }
  ...
  int buffer_len = (int)scp_strlen(buffer);
  scp_strncpy(*p, buffer, buffer_len);
  (*p) += buffer_len;
}
...
void execute_f(char **p, va_list *args, Flag flags) {
  ...
  if (isnan(number)) {
    scp_strncpy(buffer, "nan", 3);
  } else if (isinf(number) > 0) {
    scp_strncpy(buffer, "inf", 3);
  } else if (isinf(number) < 0) {
    scp_strncpy(buffer, "-inf", 4);
  } else {
...
void double_to_exp(char *buffer, long double number, Flag flags) {
  ...
  if ((int)scp_strlen(exp_count_string) < 2) {
  ...
  int i = scp_strlen(buffer);
  buffer[i] = exp_char;
  buffer[i + 1] = '\0';
  scp_strncat(buffer, exp_count_string, sizeof(exp_count_string));
}