...
START_TEST(strlen_1) {
  ...
  ck_assert_int_eq(strlen(str), scp_strlen(str));
}
END_TEST
...
START_TEST(strlen_4) { ck_assert_int_eq(strlen("\0"), scp_strlen("\0")); }
END_TEST

START_TEST(strlen_5) {
  ck_assert_int_eq(
      strlen("Po plyam, po polyam, siniy traktor edet k nam!"),
      scp_strlen("Po plyam, po polyam, siniy traktor edet k nam!"));
}
END_TEST

START_TEST(strlen_6) { ck_assert_int_eq(strlen(""), scp_strlen("")); }
END_TEST
...
START_TEST(strlen_11) { ck_assert_int_eq(strlen(""), scp_strlen("")); }
END_TEST

START_TEST(strlen_12) { ck_assert_int_eq(strlen("\t\t"), scp_strlen("\t\t")); }
END_TEST

START_TEST(strlen_13) {
  ck_assert_int_eq(strlen("\0elidacon"), scp_strlen("\0elidacon"));
}
END_TEST

START_TEST(strlen_14) {
  ck_assert_int_eq(strlen("peace \0 peace"), scp_strlen("peace \0 peace"));
}
END_TEST

Suite *test_strlen(void) {
  Suite *s = suite_create("\033[33m-=scp_strlen=-\033[0m");
